...
struct PartitionConfig
{
        ...
        //=======================================
        //= Parameters for clustering (by BSc) ==
        //= lm_ (Louvain Method) ================
        //=======================================
        /** Minimum quality (modularity) improvement necessary in the 1. phase
          of the Louvain method to perform another turn
          (another tree for the graph hierarchy). */
        double lm_minimum_quality_improvement;
        /** Number of levels/turns of performing label propagation
          before the Louvain method. */
        unsigned lm_number_of_label_propagation_levels;
        /** Number of iterations in a single label propagation
          level/turn before the Louvain method. */
        unsigned lm_number_of_label_propagation_iterations;
        /** Factor that limits the maximum size of a cluster,
          when size constrained label propagation is used.
          For the size constrained label propagation we set the maximum size
          of a cluster to (number_of_nodes / lm_cluster_coarsening_factor). */
        unsigned lm_cluster_coarsening_factor;
        /** File name for JSON output of log. */
        std::string outputLogJsonFileName;
        int do_additional_ls;
        int do_interim_print;
        int print_time;
        int do_better_evict;
